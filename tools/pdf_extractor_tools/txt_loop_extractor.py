...
dir_path = "D:/学习/面向C编译大模型的大规模循环构造技术研究/word_extraction/word_extraction/12/" # Fill the location of the directory where pdfs are stored here
store_path = "D:/学习/面向C编译大模型的大规模循环构造技术研究/benchmarks/crawled_codes/论文中的for循环代码/" # Fill the location of the directory where to store the loops here
...
for filename in filenames:
    txt_path = dir_path + filename
    this_loop_num = 0
    non_count = 0
    with open(txt_path) as pdf_file:
    ...
    while k < len(lines):
        ...
        if k >= len(lines):
            break
        if "--------------The new page is:" in lines[k]:
            pages.append(page.copy())
            page = []

    print("Storing loops from",filename)
    print("This text has %i pages." %len(pages))
...
print("Number of loops stored in",store_path,"from the given txts in",dir_path,"is %d." %all_loop_num)
